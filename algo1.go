...
// renamed: handelmarkes
func (account *p) handelmarkes(from int, snapID int, c MyChannels, accounts []*p) {
	account.mu.Lock()
		...
		go func(from int) {
			for msg := range c[from][account.id] {
				if msg < 0 {
					// marker
					snapID := -msg
					account.handelmarkes(from, snapID, c, accounts)
				} else {
					// normal transfer
					account.mu.Lock()
					account.balance += msg
...
func startSnapshot(start *p, c MyChannels, accounts []*p) {
	...
	start.handelmarkes(-1, sid, c, accounts)
}
